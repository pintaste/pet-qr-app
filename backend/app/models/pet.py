...
class Pet(SQLModel, table=True):
    ...
    age_months: Optional[int] = Field(
        default=None, description="Age in months (if birth date unknown)"
    )

    # Physical Characteristics
    color: Optional[str] = Field(
        default=None, max_length=100, description="Pet color/markings"
    )
    weight_kg: Optional[float] = Field(default=None, description="Weight in kilograms")
    microchip_id: Optional[str] = Field(
        default=None, max_length=50, description="Microchip ID"
    )

    # Description and Personality
    description: Optional[str] = Field(
        default=None, max_length=1000, description="Pet description"
    )
    personality_traits: Optional[List[str]] = Field(
        default_factory=list, sa_column=Column(JSON), description="Personality traits"
    )
    special_needs: Optional[str] = Field(
        default=None, max_length=500, description="Special needs or requirements"
    )
    ...
    profile_photo_url: Optional[str] = Field(
        default=None, max_length=500, description="Main profile photo URL"
    )
    ...
    # Owner Information
    owner_id: int = Field(
        foreign_key="shared.users.id", description="Pet owner user ID"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "medical_info": {
                    "vaccinations": {"rabies": "2024-06-15", "dhpp": "2024-06-15"},
                    "allergies": [],
                    "medications": [],
                },
                "vet_info": {
...
class QRCode(SQLModel, table=True):
    ...
    code: str = Field(
        unique=True, max_length=50, description="Unique QR code identifier"
    )
    pin: Optional[str] = Field(
        default=None, max_length=10, description="PIN for accessing pet info"
    )
    ...
    # QR Code Properties
    qr_image_url: Optional[str] = Field(
        default=None, max_length=500, description="Generated QR code image URL"
    )
    qr_data: Optional[str] = Field(
        default=None, max_length=500, description="QR code data content"
    )

    # Physical QR Code Info
    batch_id: Optional[str] = Field(
        default=None, max_length=50, description="Manufacturing batch ID"
    )
    physical_format: Optional[str] = Field(
        default="sticker",
        max_length=50,
        description="Physical format (sticker, tag, etc.)",
    )
    ...
    owner_id: Optional[int] = Field(
        default=None, foreign_key="shared.users.id", description="QR code owner"
    )
...
class PetScanLog(SQLModel, table=True):
    ...
    qr_code_id: int = Field(
        foreign_key="qr_codes.id", description="QR code that was scanned"
    )
    pet_id: Optional[int] = Field(
        default=None, foreign_key="pets.id", description="Pet associated with QR code"
    )

    # Scanner Information (anonymous)
    scanner_ip: Optional[str] = Field(
        default=None, max_length=45, description="Scanner IP address"
    )
    user_agent: Optional[str] = Field(
        default=None, max_length=500, description="Scanner user agent"
    )
    ...
    # Scan Context
    scan_method: Optional[str] = Field(
        default="qr_scan", max_length=50, description="How the scan was performed"
    )
    access_granted: bool = Field(
        default=False, description="Was access to pet info granted"
    )
    pin_verified: bool = Field(
        default=False, description="Was PIN verification successful"
    )
